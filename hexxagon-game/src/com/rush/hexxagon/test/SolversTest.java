...
public class SolversTest {
    ...
    private static int evalBoardMinMax(GameBoard board, byte playerID) {
        ArrayList<GameMove> moves = board.getPossibleMoves(playerID);

        if (moves.size() == 0) return board.evaluate(playerID);
        ...
        for (GameMove move: moves) {
            b.copy(board);
            b.move(move);
            val = Math.max(val, -evalBoardMinMax(b, otherPlayerID));
        }
        return val;
    }
    ...
    public void testMiniMax() throws Exception {
        RefGame game = new RefGame(GAME_TREE);
        final byte playerID = 0;

        int minMaxVal = evalBoardMinMax(game.mRootBoard, playerID);
